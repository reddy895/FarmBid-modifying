...
const sendInteractiveButtons = async (to, body, buttons) => {
  if (!client || !clientReady) throw new Error('Client not ready');
  ...
  const buttonPayload = limitedButtons.map((btn, idx) => ({
    buttonId: btn.id || `btn_${idx}_${Date.now()}`,
    buttonText: { displayText: btn.label || `Option ${idx + 1}` },
    type: 1
  }));

  try {
    console.log('[WhatsApp] Sending interactive buttons', { toAddress, body, limitedButtons });
    const buttonsObj = new Buttons(body, limitedButtons.map(btn => ({ id: btn.id || `btn_${Date.now()}`, body: btn.label || 'Option' })), 'Choose an option', 'Please select one');
    const result = await client.sendMessage(toAddress, buttonsObj);
    console.log(`[WhatsApp] Sent ${limitedButtons.length} buttons to ${toAddress}`);
    return result;
  } catch (err) {
    console.warn('[WhatsApp] Button send failed, falling back to text:', err.message, err.stack, { toAddress, body, limitedButtons });
    const fallback = `${body}\n\n${limitedButtons.map((b, i) => `${i + 1}. ${b.label}`).join('\n')}`;
    return sendMessageInternal({ to, body: fallback });
  }
};

// Send list message (for 4+ options)
const sendListMessage = async (to, body, sections) => {
  if (!client || !clientReady) throw new Error('Client not ready');
  const toAddress = to.endsWith('@c.us') ? to : getWhatsAppId(to);

  const payloadBody = body || 'Please select an option from the list:';
  ...
  try {
    console.log('[WhatsApp] Sending list message', { toAddress, payloadBody, formattedSections });
    const listObj = new List(payloadBody, 'Choose option', formattedSections, 'Choose from the list', 'Select one item');
    const result = await client.sendMessage(toAddress, listObj);
    console.log(`[WhatsApp] Sent list message to ${toAddress} with ${sections.length} sections`);
    return result;
  } catch (err) {
    console.warn('[WhatsApp] List send failed, falling back to text:', err.message, err.stack, { toAddress, payloadBody, formattedSections });
    let fallback = payloadBody + '\n\n';
    let idx = 1;
...
const handleFarmerMessage = async (msg) => {
  ...
  // sendReply with buttons or list
  const sendReply = async (content, buttons = null, listSections = null) => {
    const safeContent = content || 'Please choose an option below:';
    if (listSections && listSections.length > 0) {
      await sendListMessage(phone, safeContent, listSections);
    } else if (buttons && buttons.length > 0) {
      await sendInteractiveButtons(phone, safeContent, buttons);
    } else {
      await sendMessageInternal({ to: phone, body: safeContent });
    }
  };
  ...
  // Buttons now carry explicit ids: lang_en / lang_kn
  if (farmer.state === -1) {
    if (btnId === 'lang_en' || lower.includes('english') || lower.includes('en')) {
      farmer.language = 'en';
      transitionState(4);
      await sendReply(`Welcome to FarmBid, ${farmer.name}! Here are your options:`, [
        { id: 'menu_create', label: '➕ Create New Listing' },
        { id: 'menu_listings', label: '📋 View Active Listings' },
        { id: 'menu_trust', label: '📊 View Trust Score' },
        { id: 'menu_lang', label: '🌐 Change Language' }
      ]);
    } else if (btnId === 'lang_kn' || lower.includes('kannada') || lower.includes('ಕನ್ನಡ') || lower.includes('kn')) {
      farmer.language = 'kn';
      transitionState(4);
      await sendReply(`FarmBid ಗೆ ಸ್ವಾಗತ, ${farmer.name}! ಆಯ್ಕೆಗಳನ್ನು ಆರಿಸಿ:`, [
        { id: 'menu_create', label: '➕ ಹೊಸ ಪಟ್ಟಿ ರಚಿಸು' },
        { id: 'menu_listings', label: '📋 ಸಕ್ರಿಯ ಪಟ್ಟಿಗಳನ್ನು ನೋಡಿ' },
        { id: 'menu_trust', label: '📊 ವಿಶ್ವಾಸ ಅಂಕೆ ನೋಡಿ' },
        { id: 'menu_lang', label: '🌐 ಭಾಷೆ ಬದಲಾಯಿಸು' }
      ]);
    } else {
      await sendReply("Welcome to FarmBid! Please select your language:", [
  ...
  if (farmer.state === 0) {
    if (
      ...
      await sendReply(upiPrompt, [
        { id: 'upi_enter', label: '📝 Enter UPI ID now' },
  ...
  if (farmer.state === 3) {
    ...
    if (isSkip || /^[a-zA-Z0-9.\-_]{2,256}@[a-zA-Z]{2,64}$/.test(body)) {
      ...
      setTimeout(async () => {
        await sendReply(t('menu', farmer.language, { name: farmer.name }), [
          { id: 'menu_create', label: '➕ Create New Listing' },
          { id: 'menu_listings', label: '📋 View Active Listings' },
          { id: 'menu_trust', label: '📊 View Trust Score' },
          { id: 'menu_lang', label: '🌐 Change Language' }
  ...
  if (farmer.state === 4) {
    ...
    if (btnId === 'menu_lang' || lower === '4' || lower.includes('change language')) {
      transitionState(-1);
      await sendReply("Select Language / ಭಾಷೆಯನ್ನು ಆರಿಸಿ:", [
        { id: 'lang_en', label: '🇬🇧 English' },
        { id: 'lang_kn', label: 'ಕನ್ನಡ (Kannada)' }
      ]);
      return;
    ...
    // Default: show menu
    await sendReply(t('menu', farmer.language, { name: farmer.name }), [
      { id: 'menu_create', label: '➕ Create Listing' },
      { id: 'menu_listings', label: '📋 My Listings' },
      { id: 'menu_lang', label: '🌐 Change Language' }
    ]);
    setTimeout(async () => {
      await sendReply(`📊 Your Trust Score: *${farmer.trustScore}/100*`, [
        { id: 'menu_trust', label: '📊 View Details' },
  ...
  if (farmer.state === 5) {
    ...
    if (step === 'awaiting_photo') {
      if (msg.hasMedia) {
        try {
          const photoPath = await saveMedia(msg, phone);
          farmer.tempListing.images.push(photoPath);
          await sendReply(`✅ Photo received! (${farmer.tempListing.images.length} image saved).\n\nIf you want to add more photos, just send them now.\nOtherwise, choose your produce:`);
          const produceSections = [{
            title: 'Select Produce',
            rows: [
              { id: 'prod_tomato', title: '🍅 Tomatoes', description: 'Fresh farm tomatoes' },
              { id: 'prod_onion', title: '🧅 Onions', description: 'Red onions' },
              { id: 'prod_potato', title: '🥔 Potatoes', description: 'New crop' },
              { id: 'prod_chili', title: '🌶️ Green Chilies', description: 'Spicy' },
              { id: 'prod_grapes', title: '🍇 Grapes', description: 'Seedless' },
              { id: 'prod_other', title: '✏️ Type another', description: 'Manual entry' }
            ]
          }];
          await sendReply(null, null, produceSections);
          farmer.listingStep = 'awaiting_produce_name';
        } catch (err) {
    ...
    if (step === 'awaiting_produce_name') {
      ...
      const matchedProduce = Object.keys(produceOptions).find(key => cleanText.includes(key));
      if (matchedProduce) {
        farmer.tempListing.produce = produceOptions[matchedProduce];
        await sendReply(t('step_weight', farmer.language, { prod: farmer.tempListing.produce }));
        farmer.listingStep = 'awaiting_weight';
    ...
    if (step === 'awaiting_min_price') {
      if (/^\d+(\.\d+)?$/.test(body)) {
        ...
        const harvestSections = [{
          ...
          rows: [
            { id: 'harv_tomorrow', title: '🌱 Tomorrow', description: '' },
            { id: 'harv_3days', title: '📅 In 3 days', description: '' },
            { id: 'harv_1week', title: '📆 In 1 week', description: '' },
            { id: 'harv_2weeks', title: '📊 In 2 weeks', description: '' },
            { id: 'harv_1month', title: '🗓️ In 1 month', description: '' }
          ]
        }];
    ...
    if (step === 'awaiting_harvest_date') {
      ...
      if (daysAhead) {
        ...
        await sendReply('📅 When will your produce be ready?', null, harvestSections);
      }
      return;
    }
